...
function ArticelShare({ product, mode }: MenuFloatProps) {
  const { vendor, handle } = product
  ...
  const share = generateShare(text, vendor.name, handle)
  ...
  useEffect(() => {
    ...
    function handleMenuFloatSettings() {
       ...
       */
      setTimeout(() => {
        const product = document.getElementsByTagName('product')[0]
        const paragraphOffset = document.getElementsByTagName('p')[0].offsetLeft

        if (!product) return

        const productBox = product.getBoundingClientRect() as DOMRect
  ...
  useEffect(() => {
    ...
    const url = `ntve.co/${handle}`
    const tweet = text + vendor.name + url + otherCharactersInTweet
...
function generateShare(shareText: string, vendor: string, handle: string) {
  if (!shareText) return {}
  const url = `www.giftingwild.com/${handle}`

  return {
    twitter: `https://twitter.com/intent/tweet?text="${shareText}" — ${vendor} ${url}`,
    linkedin: `http://www.linkedin.com/shareArticle?mini=true&url=${url}&summary=${shareText}&title=${shareText}&source=gifting+wild`,
  }
}
