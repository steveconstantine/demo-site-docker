...
function ProductHighlight({ product, location, mode }: MenuFloatProps) {
  ...
  const share = generateShare(text, product.title, location.pathname)
  ...
  useEffect(() => {
    ...
    highlights.forEach(highlight => {
      ...
      function handleMouseOver() {
        ...
        const url = `giftingwild.com/${location.pathname}`
        const tweet =
          highlight.innerText + product.title + url + otherCharactersInTweet
...
function generateShare(shareText: string, vendor: string, shortUrl: string) {
  if (!shareText) return {}

  const url = `giftingwild.com/${shortUrl}`
